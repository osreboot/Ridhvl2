...
public final class HvlKeyboard{

	public static final String LABEL = "HvlKeyboard";
	public static final int 
	CHRONO_INIT = HvlChronology.CHRONOLOGY_INIT_LATE - (3 * HvlChronology.CHRONOLOGY_INIT_INTERVAL) + 2,
	CHRONO_EXIT = HvlChronology.CHRONOLOGY_EXIT_MIDDLE - (3 * HvlChronology.CHRONOLOGY_EXIT_INTERVAL) + 2,
	LAUNCH_CODE = 9,
	LAUNCH_CODE_RAW = 512;//2^9

	private static boolean active = false;

	static ArrayList<Character> deltaCharacters;
	static int activationsBackspace;
	...
	public static final HvlAction.A1<Boolean> ACTION_INIT = debug -> {
		...
		deltaCharacters = new ArrayList<>();
		activationsBackspace = 0;
	};
	...
	public static String getKeyName(int keyArg){
		verifyActive();

		return GLFW.glfwGetKeyName(keyArg, 0);
	}

	public static ArrayList<Character> pollCharacters(){
		verifyActive();

		ArrayList<Character> output = new ArrayList<>(deltaCharacters);
		deltaCharacters.clear();
		return output;
	}

	public static int pollActivationsBackspace(){
		int output = activationsBackspace;
		activationsBackspace = 0;
		return output;
	}
