...
public final class HvlMath {
	 ...
	 * restricting boundaries see <code>mapl</code>.
	 * 
	 * <p>
	 * 
	...
	/**
	 * A clone of Arduino's <code>map</code> function. This operation performs a 1D linear interpolation
	 * on <code>xArg</code> given a set of boundaries. The boundary arguments are restrictive, for
	 * non-restrictive boundaries see <code>map</code>.
	 * 
	 * <p>
	 * 
	 * In essence, this operation takes the relation of <code>xArg</code> to <code>inLowArg</code> and
	 ...
	 * (restrictive)
	 */
	public static float mapl(float xArg, float inLowArg, float inHighArg, float outLowArg, float outHighArg){
		return limit(map(xArg, inLowArg, inHighArg, outLowArg, outHighArg), outLowArg, outHighArg);
	}
